...
export default {
  ...
  computed: {
    Label() {
      ...
      else if (this.value && this.label && this.value[this.label]) return this.value[this.label];
    ...
    bodyStyle() {
      const labelHeight = this.$refs.dropdownGroup.clientHeight;
      return {
        'margin-top': labelHeight * (this.rounded == 'full' ? 1.13 : 0.9) + 'px',
  ...
  methods: {
    Toggle(value = null) {
      ...
      this.isOpen = value != null && typeof value == 'boolean' ? value : !this.isOpen;
    },
    onSearch(search) {
...
<template>
  <button
    :class="[$style['dropdown-group'], $style[`dropdown-rounded-${roundedLevel}`]]"
    ref="dropdownGroup"
  ...
  >
    ...
    <transition name="___airis-dropdown">
      <div v-if="isOpen" :class="$style['dropdown-body']" ref="dropdownBody" :style="bodyStyle">
